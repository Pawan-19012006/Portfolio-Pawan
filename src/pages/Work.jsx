...
const Work = () => {
  ...
  const featuredProjects = PROJECTS_DATA.slice(0, 5); // Just take top 5 for the carousel

  return (
    <div className="w-full h-full relative flex items-center bg-transparent z-10">
      <SEO title="Work — Pawan Eswaran" />
      
      {/* Massive Background Text */}
      <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-0 overflow-hidden opacity-10">
        <h1 className="text-[25vw] font-black tracking-tighter text-white/40 mix-blend-overlay">
          WORK
        </h1>
      </div>
      ...
      >
        <div className="flex items-center gap-4 lg:gap-12 py-12">
          ...
          {/* Swipe Indicator (Visible only at the end or conditionally, here just as a decorative element) */}
